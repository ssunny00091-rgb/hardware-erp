...
<div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
  <div>
    ...
    <p class="mt-2 text-sm text-gray-300">Bolo ya type karo — sale, purchase, ledger, due reminder. Bill photo daalo, automatic save.</p>
  </div>
  <button type="button" id="btn-or-menu" class="flex items-center gap-2 self-start rounded-xl border border-white/15 bg-white/10 px-3 py-2 font-semibold hover:bg-white/20" title="Assistant settings — API key / model">
    <span class="inline-block h-2.5 w-2.5 rounded-full <?= $orCfg['api_key'] !== '' ? 'bg-emerald-400' : 'bg-amber-400' ?>"></span>
    <span class="text-xl leading-none">⋮</span>
  </button>
</div>

<div id="or-panel" class="<?= ($keySavedFlash || ($keySaveFailFlash && $keySaveError === '')) ? '' : 'hidden' ?> mb-4 rounded-2xl border border-white/15 bg-slate-900/70 p-4">
  <div class="mb-3 flex items-center justify-between gap-2">
    <h2 class="font-bold">⚙️ Assistant Settings</h2>
    <button type="button" id="btn-or-close" class="rounded-lg bg-white/10 px-3 py-1 text-sm hover:bg-white/20">✖ Band karo</button>
  </div>

  <?php if ($keySavedFlash): ?>
    <div class="mb-3 rounded-xl bg-emerald-600 px-4 py-3 font-semibold text-white">
      OpenRouter key save ho gayi. Ab chat se sale / bill / ledger pooch sakte ho.
    </div>
  <?php endif; ?>
  <?php if ($keySaveFailFlash && $keySaveError === ''): ?>
    <div class="mb-3 rounded-xl bg-rose-600 px-4 py-3 font-semibold text-white">
      Key save nahi hui. Phir se paste karke try karo.
    </div>
  <?php endif; ?>

  <p id="assistant-key-status" class="mb-3 text-sm <?= $orCfg['api_key'] !== '' ? 'text-emerald-300' : 'text-amber-300' ?>">
  ...
  <p class="mb-3 text-sm text-gray-300">
    ...
    (<strong>sk-or-v1-</strong>). ChatGPT key nahi chalegi. Pehle <strong>Key save karo</strong> — internet test baad mein.
  </p>
  <form id="or-key-form" action="<?= $h(app_url('assistant.php')) ?>" method="post">
...
<section class="chat-window mb-4">
  <div class="chat-head">
    <div class="chat-avatar" aria-hidden="true">🤖</div>
    <div class="min-w-0 flex-1">
      <h2 class="truncate text-base font-bold sm:text-lg">Shop Assistant</h2>
      <p class="truncate text-xs text-white/80">Sale · Purchase · Ledger · Due · Stock — bolke ya photo se</p>
    </div>
    <span class="rounded-full bg-white/20 px-3 py-1 text-xs font-semibold">● Online</span>
  </div>

  <div id="chat-log" class="chat-log flex flex-col gap-3 overflow-y-auto p-3 sm:p-4"></div>

  <div class="border-t border-white/10 bg-slate-900/70 px-3 pb-3 pt-3 sm:px-4">
    <div id="chat-chips" class="chat-chips mb-3">
      <button type="button" class="chip" data-chip="Naya sale bill banana hai">🧾 New Sale</button>
      <button type="button" class="chip" data-chip="Supplier ka purchase bill entry karna hai">🛒 Purchase</button>
      <button type="button" class="chip" data-chip="Aaj ke due aur pending reminders dikhao">⏰ Due Today</button>
      <button type="button" class="chip" data-chip="Stock check karna hai">📦 Stock</button>
      <button type="button" class="chip" data-chip="Party ka ledger hisaab dikhana hai">📒 Ledger</button>
      <button type="button" class="chip" data-chip="Is mahine ka profit report dikhao">📈 Profit</button>
      <button type="button" class="chip" data-chip="Ek invoice ka poora detail dekhna hai">🔎 Invoice Detail</button>
      <button type="button" class="chip" data-chip="Sab reminders ki list dikhao">🔔 Reminders</button>
    </div>
...
<script src="<?= htmlspecialchars(app_url('assets/js/assistant.js'), ENT_QUOTES, 'UTF-8') ?>?v=chips2"></script>
